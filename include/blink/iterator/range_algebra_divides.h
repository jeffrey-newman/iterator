...
namespace blink
{
  namespace iterator {
    ...
    struct divides_constant_pre
    {
      divides_constant_pre(Constant a) : a(a)
      {}
    ...
    template<class Constant>
    struct divides_constant
    {
      divides_constant(Constant b) : b(b)
      {}
    ...
    struct divides_helper
    {
      ...
      using range = transform_range <divides, R1, R2>;
    };

    template<class R, class T>
    struct divides_constant_helper
    {
      using r = typename std::remove_reference<R>::type;
      using v1 = typename r::value_type;
      using v2 = typename T;
      using result_type = decltype(std::declval<v1>() / std::declval<v2>());
      using range = transform_range<divides_constant<v2>, R>;
    };

    template<class R, class T>
    struct divides_constant_pre_helper
    {
      using r = typename std::remove_reference<R>::type;
      using v1 = typename T;
      using v2 = typename r::value_type;
      using result_type = decltype(std::declval<v1>() / std::declval<v2>());
      using range = transform_range<divides_constant_pre<v1>, R>;
    };

    template<class R, class T>
    range_algebra_wrapper<typename divides_constant_helper<
      range_algebra_wrapper<R>, T>::range>
      operator/(const range_algebra_wrapper<R>& r, const T& constant)
    {
      return range_algebra_val(divides_constant_helper<range_algebra_wrapper<R>, T>
        ::range(divides_constant<T>(constant), r));
    }

    template<class R, class T>
    range_algebra_wrapper<typename divides_constant_pre_helper<
      range_algebra_wrapper<R>, T>::range>
      operator/(const T& constant, const range_algebra_wrapper<R>& r)
    {
      return range_algebra_val(divides_constant_pre_helper<range_algebra_wrapper<R>, T>
        ::range(divides_constant_pre<T>(constant), r));
    }

    template<class R1, class R2>
    range_algebra_wrapper<typename divides_helper<range_algebra_wrapper<R2>, 
      range_algebra_wrapper<R2>>::range>
      operator/(const range_algebra_wrapper<R1>& r1, 
      const range_algebra_wrapper<R2>& r2)
    {
      return range_algebra_val(divides_helper<range_algebra_wrapper<R1>, 
        range_algebra_wrapper<R2>>::range(divides{}, r1, r2));
    }
  }
}
#endif