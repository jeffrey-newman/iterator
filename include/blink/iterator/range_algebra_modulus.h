...
#ifndef BLINK_ITERATOR_RANGE_ALGEBRA_MODULUS_H_AHZ
#define BLINK_ITERATOR_RANGE_ALGEBRA_MODULUS_H_AHZ
...
namespace blink
{
  namespace iterator {
    ...
    struct modulus_constant_pre
    {
      modulus_constant_pre(Constant a) : a(a)
      {}
    ...
    template<class Constant>
    struct modulus_constant
    {
      modulus_constant(Constant b) : b(b)
      {}
    ...
    struct modulus_helper
    {
      ...
      using range = transform_range <modulus, R1, R2>;
    };

    template<class R, class T>
    struct modulus_constant_helper
    {
      using r = typename std::remove_reference<R>::type;
      using v1 = typename r::value_type;
      using v2 = typename T;
      using result_type = decltype(std::declval<v1>() % std::declval<v2>());
      using range = transform_range<modulus_constant<v2>, R>;
    };

    template<class R, class T>
    struct modulus_constant_pre_helper
    {
      using r = typename std::remove_reference<R>::type;
      using v1 = typename T;
      using v2 = typename r::value_type;
      using result_type = decltype(std::declval<v1>() % std::declval<v2>());
      using range = transform_range<modulus_constant_pre<v1>, R>;
    };

    template<class R, class T>
    range_algebra_wrapper<typename modulus_constant_helper<
      range_algebra_wrapper<R>, T>::range>
      operator%(const range_algebra_wrapper<R>& r, const T& constant)
    {
      return range_algebra_val(modulus_constant_helper<range_algebra_wrapper<R>, T>
        ::range(modulus_constant<T>(constant), r));
    }

    template<class R, class T>
    range_algebra_wrapper<typename modulus_constant_pre_helper<
      range_algebra_wrapper<R>, T>::range>
      operator%(const T& constant, const range_algebra_wrapper<R>& r)
    {
      return range_algebra_val(modulus_constant_pre_helper<range_algebra_wrapper<R>, T>
        ::range(modulus_constant_pre<T>(constant), r));
    }
    ...
    {
      return range_algebra_val(modulus_helper<range_algebra_wrapper<R1>, 
        range_algebra_wrapper<R2>>::range(modulus{}, r1, r2));
    }
  }
}
#endif